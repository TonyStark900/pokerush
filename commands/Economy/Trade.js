...
module.exports = {
  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
   ...
   let embed = new MessageEmbed()
     .setColor(color)
        .setDescription(`**<@${user.id}> you have been invited to trade by <@${message.author.id}>**`)
        .setFooter(`react with ✅ to accept and ❌ to deny`)
    ...
    const collector = msg.createReactionCollector((reaction, userx) => ['✅', '❌'].includes(reaction.emoji.name) && userx.id === user.id, { time: 60000 });
   ...
   /* const Ucollector = new MessageCollector(message.channel, m => {
     ...
     Pcollector.on("collect", async msg => {
      ...
      if(pPokemons.length > 20) return msg.channel.send(`You cannot trade over 20 things at once.`);
       ...
       if(msg.content.startsWith(`${prefix}cr`)) {
        const arg = msg.content.slice(`${prefix}cr`.length).trim().split(/ +/g); //come in discord...
        if(arg[0] == "add") {
          ...
          if(!uPokemons.some(r => r.type == "cr")) {
            uPokemons.push({type: "cr", value: num, name: "Credits"});
          }else{
          ...
          await message.channel.send(embed)
           ...
           return msg.channel.send(`It must be \`${prefix}cr <add/remove> <pokemon number>\``)
         }
        }
       ...
       if(msg.content.startsWith(`${prefix}confirm`)) {
        ...
        if(msg.content.startsWith(`${prefix}cr `)) {
        const arg = msg.content.slice(`${prefix}cr`.length).trim().split(/ +/g);
        if(arg[0] == "add") {
          if(isNaN(arg[1])) return msg.channel.send(`That is not a number.`);
          if(arg[1].includes(`-`)){
            return msg.reply("You can't add negative coins")
          ...
          embed.fields[0].name = `${message.author.username} is offering |:red_circle: `;
          embed.fields[1].name = `${user.username} is offering | :red_circle:`;
          if(!pPokemons.some(r => r.type == "cr")) {
            pPokemons.push({type: "cr", value: num, name: "Credits"});
          }else{
          ...
          await message.channel.send(embed);
           ...
           if(isNaN(arg[1])) return msg.channel.send(`That is not a number.`);
          const num = parseInt(arg[1]);
           if(!pPokemons.some(r => r.type === "cr")) return msg.channel.send(`There is no money in the trade.`);
           embed.fields[0].name = `${message.author.username} is offering | :red_circle:`;
          embed.fields[1].name = `${user.username} is offering |:red_circle: `;
           for(var i = 0; i < pPokemons.length; i++) {
         ...
         await message.channel.send(embed);
           ...
           return msg.channel.send(`It must be \`${prefix}cr <add/remove> <amount>\``)
         }
        }
