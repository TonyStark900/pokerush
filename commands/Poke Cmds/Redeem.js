...
const Alola = require('../../db/alolans.js');
...
module.exports = {
  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    let embed = new MessageEmbed()
      ...
      .addField(` \`${prefix}redeem claim\` `, ' \`To Claim 20,000 Credit.\`')
      .setColor(color)
      .setThumbnail(``)
    ...
    if (args[0].toLowerCase() == "claim" ) {
      ...
      await user.save();
      return message.channel.send(`> ✅ You claimed 20000 Credits` )
    }
        if (!args[0]) return message.channel.send(embed);
    ...
    user.redeems = user.redeems - 1;
      ...
      if (name.startsWith("alolan")) {
        ...
        var alolan = Alola.find(r => r.name.toLowerCase() === name.toLowerCase());
      };
