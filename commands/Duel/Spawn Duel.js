...
let url1 = user.pokemons[selected].url
    ...
    if (user.shname !== null && poke.name.toLowerCase() == user.shname.toLowerCase()) ++user.shcount
     ...
     let move = ("Thunder Bolt","Hyper Beam","Posion","Psystrike","Self Destruct","Scratch","Twister","Leaf Blade","Absorb")
      ...
      await user.save();
      await user.markModified("pokemons")
                            //Battle ended and Level ${poke.level} ${poke.name} won the Battle and was rewarded with 10  <:moneybag:983603699618316288> Coins
                        } else if (hp1 < 1) {
                           Duelcollector.stop();
                          ...
                          embed5 = new Discord.MessageEmbed()
                            ...
                            return message.channel.send(embed5)
                                      user.balance = user.balance + 100
      await user.save();
                            //Battle ended and ${message.author} won the Battle and was rewarded with 10  <:moneybag:983603699618316288> Coins
                        }
                    }
